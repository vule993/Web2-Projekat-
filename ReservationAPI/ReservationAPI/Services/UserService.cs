...
namespace ReservationAPI.Services
{
    public class UserService : IUserRepository
    {
        ...
        public async Task<object> Get(string email)
        {
            ...
            List<UserFriend> friends = _context.Friends.ToList();
        ...
        public async Task<List<UserModel>> GetAllFriends(string email, User loggedUser)
        {
            ...
            List<UserFriend> friendLinks = _context.Friends.ToList();


            foreach (var friendLink in friendLinks)
            {
                var friend = (friendLink.SourceUser.Email == loggedUser.Email) 
                    ? await _userManager.FindByEmailAsync(friendLink.DestinationUser.Email) 
                    : await _userManager.FindByEmailAsync(friendLink.SourceUser.Email);
        ...
        public async Task<object> AddFriend(AddFriendModel newFriends)
        {
            ...
            var friends = _context.Friends.ToList().Find(x=> x.SourceUserId == user1.Id && x.DestinationUserId==user2.Id);
        ...
        public async Task<object> RemoveFriend(AddFriendModel unFriends)
        {
            ...
            UserFriend friendToRemove = _context
                .Friends
                .ToList()
                .Find(
                    x => x.SourceUserId == user.Id && x.DestinationUserId == friend.Id ||
                    x.SourceUserId == friend.Id && x.DestinationUserId == user.Id
                );
